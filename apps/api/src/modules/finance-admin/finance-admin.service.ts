...
export class FinanceAdminService {
  ...
  async inboundList(limit = 50) {
    // ADR 0011 single-rail: brand-funding rows are now TradeSafe transactions.
    // `stitch_payment_links` was dropped in the 2026-04-24 migration.
    return this.prisma.tradeSafeTransaction.findMany({
      take: limit,
      orderBy: { createdAt: 'desc' },
  ...
  /**
   * Platform-wide outbound-payout listing for SUPER_ADMIN triage.
   *
   * **Phase 4 deferred (2026-04-24):** the `stitch_payouts` table was
   * dropped as part of the single-rail Stitch-deletion cutover. The
   * TradeSafe-native payout table lands in Phase 4 alongside the
   * submission-approval → auto-payout trigger. Returns an empty page
   * until then — the Finance dashboard shows "no payouts yet".
   */
  async listPayouts(page = 1, limit = 25) {
    ...
    return {
      ...
      meta: { page, limit: take, total: 0, totalPages: 1 },
    };
  }
