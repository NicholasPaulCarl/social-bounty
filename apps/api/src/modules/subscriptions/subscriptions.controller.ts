...
/**
 * Subscription lifecycle API (ADR 0011 — TradeSafe unified rail).
 *
 * Post-cutover: `POST /subscription/upgrade` removed. TradeSafe has no
 * recurring-subscription primitive, so the live Pro-tier upgrade CTA is
 * paused until a subscription-capable PSP is chosen (ADR 0011 §7). All
 * other lifecycle routes (subscribe / cancel / reactivate / payments /
 * getSubscription) remain provider-agnostic and stay live for tier
 * display + auto-downgrade management.
 */
