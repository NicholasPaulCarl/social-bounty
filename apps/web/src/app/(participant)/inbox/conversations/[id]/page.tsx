...
function MessageBubble({ msg, isMine }: { msg: InboxMessageResponse; isMine: boolean }) {
    ...
    return (
      <div className="flex justify-center my-4">
        <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-elevated border border-glass-border">
          <Info size={12} strokeWidth={2} className="text-text-muted" />
    ...
    return (
      <div className={`flex items-start gap-3 ${isMine ? 'flex-row-reverse' : ''}`}>
        <div className="w-8 h-8 rounded-full bg-elevated flex items-center justify-center shrink-0 opacity-40">
          <User size={12} strokeWidth={2} className="text-text-muted" />
        </div>
        <div className={`max-w-[70%] px-4 py-2.5 rounded-xl text-xs italic text-text-muted bg-elevated border border-glass-border ${isMine ? 'rounded-tr-sm' : 'rounded-tl-sm'}`}>
          Message deleted
        </div>
      </div>
    );
  ...
  return (
    <div className={`flex items-end gap-3 ${isMine ? 'flex-row-reverse' : ''}`}>
      {/* Avatar — mine uses pink brand; theirs uses slate neutral
          (was blue — standalone-blue removal) */}
      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 mb-0.5
        ${isMine ? 'bg-pink-600/20 text-pink-600' : 'bg-slate-100 text-slate-700'}`}>
      ...
      <div className={`max-w-[70%] space-y-1 ${isMine ? 'items-end' : 'items-start'} flex flex-col`}>
        ...
        {msg.attachmentUrl && (
          <a
            ...
            className="flex items-center gap-2 text-xs text-pink-600 hover:text-pink-700 transition-colors px-1"
          >
            ...
            <div className="flex items-center flex-wrap gap-x-3 gap-y-1 mt-1">
              ...
              {data.referenceId && (
                <span className="text-xs text-text-muted flex items-center gap-1">
                  ...
                  Ref: <span className="font-mono tabular-nums">{data.referenceId.slice(0, 8)}…</span>
                </span>
              )}
            </div>
          </div>
      ...
      <div className="flex-1 glass-card mb-4 overflow-y-auto p-5 space-y-4 animate-fade-up">
        {data.messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <MessagesSquare size={32} strokeWidth={2} className="text-text-muted mb-3" />
      ...
      <div className="glass-card px-4 py-3 animate-fade-up shrink-0">
        <div className="flex items-end gap-3">
          ...
          <Button
            ...
            className="bg-pink-600 border-pink-600 text-background hover:bg-pink-700 shrink-0 mb-0.5"
            onClick={handleSend}
