          ...
          <div className="bg-white rounded-lg border border-neutral-200 p-6">
            ...
            {auditLogs.length > 0 ? (
              <div className="space-y-4">
                {auditLogs.map((log) => (
                  <div key={log.id} className="flex items-start gap-3 pb-4 border-b border-neutral-100 last:border-0">
                    <div className="w-8 h-8 rounded-full bg-primary-100 flex items-center justify-center flex-shrink-0">
                      <i className="pi pi-history text-primary-600 text-sm" />
                    </div>
            ...
            {healthChecks.length > 0 ? (
              <div className="space-y-3">
                {healthChecks.map((check) => (
                  <div key={check.service} className="flex items-center gap-3 p-3 rounded-lg bg-neutral-50">
                    <span className={`w-3 h-3 rounded-full flex-shrink-0 ${
                      check.status === 'healthy' ? 'bg-green-500' :
                      check.status === 'degraded' ? 'bg-yellow-500' : 'bg-red-500'
                    }`} />
                    <div>
                      <p className="text-sm font-medium text-neutral-700">{check.service}</p>
                      <p className="text-xs text-neutral-400">{check.responseTime}ms</p>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-sm text-neutral-400">Loading health data...</p>
            )}
            {healthData && (
