  ...
  return (
    <>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <span className="font-mono text-sm text-on-surface-variant">ID: {submissionId.slice(0, 8)}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {reviewHistory.length > 0 && (
            <Card>
              <h3 className="text-lg font-semibold text-on-surface mb-4">Review History</h3>
              <div className="relative pl-6 space-y-6">
                <div className="absolute left-2 top-2 bottom-2 w-0.5 bg-surface-container-low" />
                {reviewHistory.map((entry, i) => (
                  <div key={i} className="relative">
                    <div className="absolute -left-4 top-1 w-3 h-3 rounded-full bg-surface-container border-2 border-white" />
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      ...
                      {entry.changedBy && (
                        <span className="text-xs text-on-surface-variant">by {entry.changedBy}</span>
                      )}
                    </div>
                    {entry.note && (
                      <p className="text-sm text-on-surface mt-1">{entry.note}</p>
                    )}
                  </div>
                ))}
              </div>
        ...
        <div className="space-y-6">
          {/* Participant Info */}
          <Card>
            <h3 className="text-lg font-semibold text-on-surface mb-4">Participant</h3>
          ...
          <Card>
            ...
            <dl className="space-y-3">
              <div>
                <dt className="text-sm text-on-surface-variant">Title</dt>
                <dd
              ...
              {submission.bounty?.rewardValue && (
                <div>
                  <dt className="text-sm text-on-surface-variant">Reward</dt>
                  <dd className="text-sm font-medium font-mono text-on-surface">
          ...
          <Card>
            ...
            <dl className="space-y-3">
              ...
              <div>
                <dt className="text-sm text-on-surface-variant">Last Updated</dt>
                <dd className="text-sm font-medium font-mono text-on-surface">
