...
const REWARD_TYPE_OPTIONS = [
  { label: 'Cash', value: RewardType.CASH, icon: 'pi pi-money-bill', iconColor: 'text-success' },
  { label: 'Product', value: RewardType.PRODUCT, icon: 'pi pi-box', iconColor: 'text-primary' },
  { label: 'Service', value: RewardType.SERVICE, icon: 'pi pi-wrench', iconColor: 'text-warning' },
  { label: 'Other', value: RewardType.OTHER, icon: 'pi pi-gift', iconColor: 'text-on-surface-variant' },
];
  ...
  return (
    <>
      <div className="flex items-center gap-3 mb-4">
        <label className="text-sm font-semibold text-on-surface">Currency</label>
        <Dropdown
          value={currency}
      ...
      <div className="hidden md:block border border-outline-variant rounded-lg overflow-hidden">
        <div className="grid grid-cols-[10rem_1fr_9rem_3rem] gap-3 p-3 bg-surface border-b border-outline-variant">
          <span className="text-xs font-semibold text-on-surface-variant uppercase">Type</span>
          <span className="text-xs font-semibold text-on-surface-variant uppercase">Name</span>
          <span className="text-xs font-semibold text-on-surface-variant uppercase">Value ({currencySymbol})</span>
          <span />
        </div>
        {rewards.map((reward, index) => (
          <div
            key={index}
            className="grid grid-cols-[10rem_1fr_9rem_3rem] gap-3 p-3 border-b border-outline-variant last:border-b-0 items-center"
          >
            <Dropdown
      ...
      <div className="md:hidden space-y-3">
        {rewards.map((reward, index) => (
          <div key={index} className="space-y-3 p-4 border border-outline-variant rounded-lg">
            <div className="flex justify-between items-center">
              <span className="text-sm font-semibold text-on-surface">Reward {index + 1}</span>
              {rewards.length > 1 && (
                <Button
                  icon="pi pi-trash"
                  text
      ...
      <div className="flex items-center justify-between mt-3">
        ...
        {rewards.length >= BOUNTY_REWARD_LIMITS.MAX_REWARD_LINES && (
          <small className="text-xs text-on-surface-variant">Maximum {BOUNTY_REWARD_LIMITS.MAX_REWARD_LINES} reward lines</small>
        )}
      </div>

      <div className="flex justify-end mt-4 pt-3 border-t border-outline-variant">
        <div className="text-right">
          <span className="text-xs text-on-surface-variant uppercase tracking-wider">Total Reward Value</span>
          <p className="text-lg font-bold text-on-surface mt-0.5">
            <span className="text-on-surface-variant text-base font-normal mr-1">{currencySymbol}</span>
