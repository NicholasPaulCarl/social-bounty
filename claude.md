...
- **Single-rail TradeSafe architecture** (2026-04-24, commit <TBD by Wave 2 integration>) — All historical Stitch, Stripe, and Peach Payments code deleted. Bounty funding now routes through `TradeSafePaymentsService.createBountyFunding` (Phase 3 inbound cutover — per ADR 0011). The `FUNDS_RECEIVED` TradeSafe webhook posts the `bounty_funded` ledger group and flips `Bounty.status: DRAFT → LIVE`. Subscription upgrade backend fully removed; UI retains a "Pro subscriptions coming soon" placeholder. R24 (TradeSafe prod OAuth creds) remains the sole blocker to real-money flow — `TRADESAFE_MOCK=true` on Render until creds arrive. Historical destructive migration `<timestamp>_drop_stitch_tables` dropped `stitch_payment_links`, `stitch_payouts`, `stitch_beneficiaries`, `stitch_subscriptions` tables; the `PayoutRail.STITCH` enum value is left in place (zero-risk, no rows reference it post-drop).
