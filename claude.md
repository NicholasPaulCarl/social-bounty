...
HEAD: `5e8ec37 test(submissions): visibility scheduler test coverage` (7 commits past `16e2095` on the `phase2-postvisibility-recheck` branch — pending merge to main). Test state: **1542 tests across 91 suites, 100% green** — api 1251 / 82 suites + web 291 / 9 suites (Hard Rule #4 held). API count grew 1235 → 1251 (+10 from Phase 2A visibility scheduler suite, +6 from Phase 2C stuck-PENDING recovery suite). Web count grew 266 → 291 (+25 from Phase 2B bounty-preview-checks helper + panel preview-mode tests). Working tree clean.
