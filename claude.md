...
HEAD: `<TBD by Wave 2 integration>` on main — single-rail TradeSafe cutover landed per ADR 0011; the historical Stitch / Stripe / Peach integrations were purged from code and docs in the same wave. Test state: **~1280 tests across ~60 suites, 100% green** (estimate — backend agent 1A deleted ~371 legacy-provider tests, frontend 1C dropped ~4 tests, API count now ~976 / 60 suites + web 300 / 10 suites; Hard Rule #4 held). Working tree clean. Destructive migration `<timestamp>_drop_stitch_tables` (historical naming) dropped the legacy `stitch_payment_links`, `stitch_payouts`, `stitch_beneficiaries`, `stitch_subscriptions` tables; the `PayoutRail.STITCH` enum value is left in place (zero-risk, no rows reference it post-drop).
