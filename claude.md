...
HEAD: `f827879 Merge branch 'hunter-profile-update' — mobile UX pass + bounty form polish`. Test state: **1457 tests across 85 suites, 100% green** — api 1191 / 77 suites + web 266 / 8 suites (Hard Rule #4 held). API count dropped 1193 → 1191 (two visibility-acknowledgment tests deleted along with the gate); web count grew 263 → 266 (three CASH-reward tests added). Working tree now genuinely clean — `*.tsbuildinfo` and `apps/web/test-results/` are gitignored as of `867f130` (the four build artefacts that had slipped into the repo were untracked in the same commit). Staging Supabase DB is now fully migrated — the 6 pending migrations from batch 13A were applied 2026-04-17.
