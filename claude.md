...
HEAD: `f0b0bb5 chore: add .claude/launch.json for Claude Preview web server`. Test state: **1455 tests across 85 suites, 100% green** — api 1192 / 77 suites + web 263 / 8 suites (Hard Rule #4 held). Working tree now genuinely clean — `*.tsbuildinfo` and `apps/web/test-results/` are gitignored as of `867f130` (the four build artefacts that had slipped into the repo were untracked in the same commit).
