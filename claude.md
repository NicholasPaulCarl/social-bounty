...
HEAD: `da71b0a fix: expose bounty accessType in list + detail API responses`. Test state: **1453 tests across 85 suites, 100% green** — api 1190 / 77 suites + web 263 / 8 suites (Hard Rule #4 held). Working tree is clean apart from build artifacts (`tsconfig.tsbuildinfo`, `tsconfig.build.tsbuildinfo`, `apps/web/test-results/`).
