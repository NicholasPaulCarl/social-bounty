...
import { ArrowLeft, Maximize2, GripVertical, Layout, Smartphone, Tablet, Monitor, Search, FileText, Briefcase, ClipboardCheck, Shield, Circle } from 'lucide-react'
    ...
    return React.lazy(async () => {
      try {
        ...
        const ShellWrapper = ({ children }: { children?: React.ReactNode }) => {
          ...
          // Known icons for common nav labels
          const iconMap: Record<string, typeof Circle> = {
            'Bounty Marketplace': Search,
            'My Submissions': FileText,
            'Bounty Management': Briefcase,
            'Review Center': ClipboardCheck,
            'Admin Panel': Shield,
          }
          ...
          const navigationItems = specNavItems.length > 0
            ...
            : [
                { label: 'Dashboard', href: '/', icon: Circle, isActive: true },
