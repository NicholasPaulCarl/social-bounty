...
> **Extended 2026-04-24 by ADR 0011** — from outbound-only to unified inbound + outbound. The hybrid Stitch-inbound / TradeSafe-outbound posture described below is historical; single-rail TradeSafe is the current architecture.

**Status:** Accepted
**Date:** 2026-04-15
**Supersedes:** ADR 0007 (Peach Payments for Hunter Payouts) <!-- historical -->
**Extended by:** ADR 0011 (TradeSafe Unified Rail)

## Context

ADR 0007 (earlier today) locked Peach Payments as the outbound rail for hunter payouts because Stitch Express has no multi-recipient payout surface. That decision has been revised. <!-- historical -->
...
**Use TradeSafe for outbound hunter payouts.** Stitch Express remains the inbound rail for brand bounty funding (unchanged). <!-- historical -->
...
- ADR 0007 (Peach Payments for Hunter Payouts) — now superseded; Peach is no longer being integrated. <!-- historical -->

This remains a **hybrid model**:
- **Inbound (brand → platform reserve):** Stitch Express hosted checkout. Shipped and live-tested. <!-- historical -->
- **Outbound (platform reserve → hunter):** TradeSafe. To be integrated in a dedicated workstream. Out of scope for this current Stitch work. <!-- historical -->

## Constraints on the integration (when it happens)

Same non-negotiables apply as would have for Peach: <!-- historical -->
...
3. `WebhookProvider` enum gets a new `TRADESAFE` value (not `PEACH`). Webhook router gains a `/webhooks/tradesafe` route. <!-- historical -->
4. `StitchPayout` and `StitchBeneficiary` tables get TradeSafe-aware peers (or renamed provider-agnostic — the integration ADR decides). <!-- historical -->
5. `TRADESAFE_*` env vars replace the would-have-been `PEACH_*` vars. <!-- historical -->
...
- The Stitch-side workarounds already in place (synthetic `local:…` beneficiary ids, disabled `PAYOUTS_ENABLED` flag, BeneficiaryService live-mode guard) all stand. The integration will replace these paths. <!-- historical -->
- `// PEACH MIGRATION:` comment markers across payout call sites are rewritten to `// TRADESAFE MIGRATION:` — see batch 6 backend agent. <!-- historical -->
...
- **No code shipped today moves.** Inbound is still Stitch, ledger is provider-agnostic, finance admin dashboard + reconciliation + KB automation are all untouched. <!-- historical -->
...
- **The Peach relationship is unwound.** No Peach sandbox or commercial work has been started under ADR 0007, so the unwind is a documentation-only change. <!-- historical -->

## Action items (closed under this ADR)

- [x] This ADR written.
- [ ] Flip `// PEACH MIGRATION:` → `// TRADESAFE MIGRATION:` on all 8 call sites — covered in batch 6 backend. <!-- historical -->
