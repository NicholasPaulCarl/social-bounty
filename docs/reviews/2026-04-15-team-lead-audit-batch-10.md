...
| **R26** | `SubscriptionTier` enum missing from migrations (fresh-DB break) | **Critical (deferred fix)** | Pre-existing in `20260415143053_stitch_express` (NOT introduced by 10B), now amplified because 10B references the same enum. **Action:** prepend `CREATE TYPE "SubscriptionTier" AS ENUM ('FREE','PRO');` to the start of `20260415143053_stitch_express/migration.sql`. Safe because no environment has actually replayed that migration without the type already present (it was created by an earlier `prisma db push` in dev). After fix, verify with `prisma migrate diff --from-empty --to-migrations …`. |
