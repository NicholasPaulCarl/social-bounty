...
| **R27** | **`subscriptions` and `subscription_payments` tables missing from migrations** | **High** | **Fixed in batch 12A, commit <sha-placeholder>** | Same root cause as R26 (drift via `db push`). `20260415190000_stitch_subscription_upgrade` references both tables; neither is created by any committed migration. Runtime impact today: zero (existing envs have the tables). **Resolution (batch 12A, Backend agent):** added new pre-conditioning migration `packages/prisma/migrations/20260415142500_subscription_tables/migration.sql` (timestamp strictly earlier than `20260415143000_subscription_tier_enum` and `20260415143053_stitch_express`) that idempotently creates the two tables, their indexes and foreign keys, and the four supporting enums (`SubscriptionTier`, `SubscriptionEntityType`, `SubscriptionStatus`, `SubscriptionPaymentStatus` — none of which were created by any committed migration). Each CREATE TABLE / CREATE INDEX uses `IF NOT EXISTS`; each CREATE TYPE and ADD CONSTRAINT is wrapped in `DO $$ … EXCEPTION WHEN duplicate_object THEN null; END $$`. The UNIQUE INDEX on `subscription_payments("providerPaymentId")` is intentionally NOT created here — it remains owned by `20260415190000_stitch_subscription_upgrade`. Historical migrations were **NOT edited**. Verified by running `prisma migrate deploy` end-to-end against a fresh local Postgres DB (all 10 migrations apply cleanly, exit 0); re-running on the same DB yields `No pending migrations to apply`; re-running the new migration's SQL directly via psql on a populated DB is a safe no-op (expected `NOTICE: relation … already exists, skipping` lines, exit 0). Note: `prisma migrate diff` against the schema reveals additional pre-existing drift (users.passwordHash, many index/FK additions, ~16 tables and ~16 enums not created by any committed migration) — out of scope for R27, tracked separately. Spec doc fix (`md-files/payment-gateway.md` `PAID` → `SETTLED` wording) remains a separate non-blocking docs ticket. |
