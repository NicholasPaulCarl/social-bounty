# Agent team roster

> 107 nodes · cohesion 0.04

## Key Concepts

- **SUPER_ADMIN role** (36 connections) — `md-files/page-spec/`
- **Page Specs Index** (21 connections) — `md-files/page-spec/README.md`
- **Financial Kill Switch** (16 connections) — `md-files/page-spec/admin-finance.md`
- **ADR 0006 — Compensating Entries Bypass the Financial Kill Switch** (14 connections) — `docs/adr/0006-compensating-entries-bypass-kill-switch.md`
- **ADR 0010 — Auto-Refund on PostVisibility Failure** (13 connections) — `docs/adr/0010-auto-refund-on-visibility-failure.md`
- **Finance Overview admin page** (12 connections) — `md-files/page-spec/admin-finance.md`
- **/admin/brands/[id]** (11 connections) — `md-files/page-spec/admin-brands-id.md`
- **Visibility Failures admin page** (11 connections) — `md-files/page-spec/admin-finance-visibility-failures.md`
- **ADR 0005 — Ledger Idempotency via Header Table** (10 connections) — `docs/adr/0005-ledger-idempotency-via-header-table.md`
- **/admin/bounties/[id]** (9 connections) — `md-files/page-spec/admin-bounties-id.md`
- **Admin Submission Detail page** (9 connections) — `md-files/page-spec/admin-submissions-id.md`
- **Admin Wallet Detail page** (9 connections) — `md-files/page-spec/admin-wallets-userid.md`
- **Admin Withdrawals page** (9 connections) — `md-files/page-spec/admin-withdrawals.md`
- **/admin/audit-logs** (8 connections) — `md-files/page-spec/admin-audit-logs.md`
- **/admin/finance/groups/[transactionGroupId]** (8 connections) — `md-files/page-spec/admin-finance-groups-transactiongroupid.md`
- **/admin/finance/payouts** (8 connections) — `md-files/page-spec/admin-finance-payouts.md`
- **Admin User Detail page** (8 connections) — `md-files/page-spec/admin-users-id.md`
- **ledger.service.ts** (8 connections) — `/Users/nicholasschreiber/social-bounty/apps/api/src/modules/ledger/ledger.service.ts`
- **/admin/dashboard** (7 connections) — `md-files/page-spec/admin-dashboard.md`
- **/admin/disputes/[id]** (7 connections) — `md-files/page-spec/admin-disputes-id.md`
- **/admin/finance/overrides** (7 connections) — `md-files/page-spec/admin-finance-overrides.md`
- **/admin/finance/refunds** (7 connections) — `md-files/page-spec/admin-finance-refunds.md`
- **Brand Reserves page** (7 connections) — `md-files/page-spec/admin-finance-reserves.md`
- **ADR 0002 — Wallet as Read-Model Projection over LedgerEntry** (7 connections) — `docs/adr/0002-wallet-read-model-projection.md`
- **Append-only double-entry ledger** (7 connections) — `md-files/page-spec/admin-finance.md`
- *... and 82 more nodes in this community*

## Relationships

- [[Admin operations & overrides]] (21 shared connections)
- [[Brand funding & form state]] (17 shared connections)
- [[ConfirmAction.tsx]] (5 shared connections)
- [[layout.tsx]] (5 shared connections)
- [[Admin page routes]] (4 shared connections)
- [[Finance admin & KB]] (2 shared connections)
- [[BrandsService class]] (2 shared connections)
- [[ADR headline docs]] (2 shared connections)
- [[TradeSafe client & payouts]] (1 shared connections)
- [[Bounties & Stitch payments]] (1 shared connections)

## Source Files

- `/Users/nicholasschreiber/social-bounty/apps/api/src/modules/ledger/ledger.service.spec.ts`
- `/Users/nicholasschreiber/social-bounty/apps/api/src/modules/ledger/ledger.service.ts`
- `/Users/nicholasschreiber/social-bounty/apps/api/src/modules/refunds/refunds.service.ts`
- `/Users/nicholasschreiber/social-bounty/apps/api/src/modules/submissions/__tests__/submission-visibility.scheduler.spec.ts`
- `/Users/nicholasschreiber/social-bounty/apps/api/src/modules/submissions/submission-visibility.scheduler.ts`
- `claude.md`
- `docs/INCIDENT-RESPONSE.md`
- `docs/adr/0002-wallet-read-model-projection.md`
- `docs/adr/0005-ledger-idempotency-via-header-table.md`
- `docs/adr/0006-compensating-entries-bypass-kill-switch.md`
- `docs/adr/0010-auto-refund-on-visibility-failure.md`
- `docs/architecture/api-contracts.md`
- `docs/architecture/database-schema.md`
- `docs/architecture/frontend-strategy.md`
- `docs/architecture/security-and-rbac.md`
- `docs/architecture/sitemap.md`
- `docs/backlog/mvp-backlog.md`
- `docs/backlog/release-plan.md`
- `docs/deployment/stitch-express-live-readiness.md`
- `docs/features/AGENCY-FEATURE-SPEC.md`

## Audit Trail

- EXTRACTED: 462 (90%)
- INFERRED: 50 (10%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [[index]] to navigate.*